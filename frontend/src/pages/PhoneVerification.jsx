...
//     className="phone-verification bg-purple-900 h-screen flex flex-col items-center justify-center">
...
const PhoneVerification = () => {
  ...
  const handlePhoneSubmit = async (e) => {
    e.preventDefault();
    try {
      // Send phone number to backend to trigger sending the verification code
      await axios.post("http://localhost:5000/api/phone/sign-up", {
  ...
  const handleVerificationSubmit = async (e) => {
    e.preventDefault();
    if (codeExpired) {
      alert("The code has expired. Please request a new code.");
      return;
    }
    try {
      ...
      {!isCodeSent ? (
        ...
        <form onSubmit={handleVerificationSubmit} className="flex flex-col gap-4">
          ...
          <div className="text-gray-500">
            {codeExpired ? (
              <span className="text-red-500">Code expired. Please request a new code.</span>
            ) : (
